...
if grep -q '^disable_fw_kms_setup=1$' "$config_file"; then
  log "Firmware-EDID-Override ist deaktiviert (disable_fw_kms_setup=1)"
else
  die "disable_fw_kms_setup=1 wurde nicht korrekt gesetzt."
fi

video_count="$(grep -o 'video=[^ ]*' "$cmdline_file" | wc -l)"
[[ "$video_count" -eq 1 ]] || die "cmdline.txt enthält weiterhin mehrere video=-Parameter."
